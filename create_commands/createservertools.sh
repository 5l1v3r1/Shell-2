...
function webmin {
  ...
  echo "[Downloading] Grabbing $_link"

  wget $_link -P /tmp/ -q
  echo "[Installing] Installing $_file from /tmp/"
  dpkg -i /tmp/$_file; apt install -f
...
function checkpackages {
  ...
  do
    if [ $(dpkg-query -W -f='${Status}' $item 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
      echo -e "\e[1;31m[ MISSING ]\e[0m $item"
        printf "Do you wish to install this now? (y/n) "; read _yn
      if [[ $_yn =~ ^[Yy]$ ]]; then
        echo -e "\e[1;33m[ INSTALLING ] $item"
        ...
        echo -e "\e[1;33m[ DONE ] $item\e[0m"
      fi
    else
      # Already installed
      echo -e "\e[1;32m[OK]\e[0m $item"
    fi
  done
